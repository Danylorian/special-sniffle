...
import { listCaissesWithBalance, getSettings } from "@/lib/queries";
import { createCaisseAction } from "@/app/actions";
import { formatAmount } from "@/lib/money";
import ArchiveCaisseButton from "@/components/ArchiveCaisseButton";

export default function CaissesPage() {
  const settings = getSettings();
  const caisses = listCaissesWithBalance();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-2xl border border-black/10 dark:border-white/10 p-4">
        ...
        <form action={createCaisseAction} className="flex flex-col gap-3">
          <input
            name="name"
            type="text"
            required
